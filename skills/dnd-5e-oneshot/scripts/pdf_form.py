"""Shared helpers for filling AcroForm PDF forms and stamping portrait images onto them."""
import subprocess
import tempfile
...
def fill_text_fields(input_pdf: Path, output_pdf: Path, values: dict[str, str]) -> None:
    ...
    German umlauts/eszett and field names containing "&" or trailing spaces (this
    sheet has several, e.g. "Race ") round-trip correctly without manual escaping.
    ...
    page on several fields) even though the stored field value is correct — see
    test_fill_text_fields_renders_umlauts_on_the_page.
    """
    entries = "\n".join(
